                           ...
                           col, lty, lwd, digits = 4, event = 1)
{
  ...
  { # plot cumulative incidence functions
    ...
    matplot(time, base::t(x$est), type="s", ylim = c(0,1), xlim = c(min(ttime), max(ttime)),
            xlab = xlab, ylab = ylab, axes=F, #xaxs="i", yaxs="i", 
            col = col, lty = lty, lwd = lwd)
    text(0.85*max(ttime), 0.9, paste0("Gray's P-value: ", round(tests[rownames(tests) == event, 2], 4)))
    # Add Y-axis as is
    axis(2)
    axis(side=1,at=ttime,labels=ttime)
  ...
  { if(level < 0 | level > 1) 
    ...
    { matplot(time, cbind(x$est[j,], lower[j,], upper[j,]), type="s", 
              xlab = xlab, ylab = ylab, axes=F, #xaxs="i", yaxs="i", 
              ylim = c(0,1), xlim = c(min(ttime), max(ttime)), col = col[j], lwd = lwd[j], lty = c(1,3,3))
      # Add Y-axis as is
      axis(2)
      axis(side=1,at=ttime,labels=ttime)
