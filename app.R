                      ...
                      fluidRow(
                        column(2,
                               ...
                               wellPanel(
                                 ...
                                 radioButtons("event", "Event Code:", list("1" = 1, "2" = 2, "0" = 0), selected = 1),
                               ...
                               wellPanel(
                                 ...
                                 selectInput("show_vars36", "Select multiple Variables to Generate Univariate Survival Association Table", 
                                             c("AGE", "RACE"), choices=c("AGE", "RACE"), multiple = TRUE),
                                 radioButtons("event2", "Event Code:", list("1" = 1, "2" = 2, "0" = 0), selected = 1),
                      ...
                      fluidRow(
                        column(2,
                               ...
                               wellPanel(
                                 ...
                                 h5("Hit Run & Generate random number button above to display output on main panel."),
...
server <- function(input, output, session){
  ...
  output$kmplot <- renderPlot({
    ...
    for (j in 1:ncol(data2)) {
      if (class(data2[, j]) == "integer" & length(table(data2[, j])) < 3) 
  ...
  output$out2 <- renderDataTable({
    data2 <- data_input2()
    for (j in 1:ncol(data2)) {
      if (class(data2[, j]) == "integer" & length(table(data2[, j])) < 10) 
        data2[,j] <- as.factor(data2[,j])
      else data2[,j] = data2[, j]
    } 
  ...
  data_input3 <- reactive({
    if(input$file3 == 'Example3'){
      ...
      else if(grepl(".csv", inFile[1])) { d3 = read.csv(as.character(inFile$datapath), header = TRUE, sep = ",", stringsAsFactors = F, as.is = T, fill = T) }
      else if(grepl(".txt", inFile[1])) { d3 = read.table(as.character(inFile$datapath), header = TRUE, sep = "\t", stringsAsFactors = F, as.is = T, fill = T) }
    }
    else 
      return(NULL)
    # dim(data)
  ...
  output$cifplot <- renderPlot({
    ...
    if (is.null(xvar) | is.null(time) | is.null(censor)) {
      ...
      return(NULL)
    } else if (class(xvar) == "factor" & class(time)=="numeric"){
      if (input$points == 0) {
        xlabel <- c("Years", "Months", "Days")[as.numeric(input$time2)]
  ...
  output$out3 <- renderDataTable({
    ...
    for (i in 1: ncol(xvar)){
      ...
      if (is.null(x) | is.null(time) | is.null(censor)) {
        ...
        return(NULL)
      } else if (class(x) == "factor" & class(time)=="numeric"){
        Variable <- c(colnames(xvar)[i], rep("", (length(levels(x))-1)))
        #x <- C(x, contr.treatment, base=3)
        ...
        lower95 = round(sum$conf.int[, 3], 2)
        upper95 = round(sum$conf.int[, 4], 2)
        gray.p.value = ifelse(cuminc(time, censor, factor2ind(x))$Tests[1, "pv"] < 0.001, "<0.001", paste0(round(cuminc(time, censor, factor2ind(x))$Tests[1, "pv"], 4)))
        ...
        res[[i]] <- coxres
      }else if (class(x) %in% c("integer", "numeric") & class(time)=="numeric"){
        Variable <- colnames(xvar)[i]
  ...
  output$landmarkplot <- renderPlot({
    ...
    for (j in 1:ncol(data4)) {
      if (class(data4[, j]) == "integer" & length(table(data4[, j])) < 3) 
